// Admin — Euphoria Ladies Gallery manager (photos + YouTube videos).

import { useState, useEffect, useRef, useCallback } from 'react';
import { Plus, Trash2, Eye, EyeOff, Loader2, X, Save, Play } from 'lucide-react';
import {
  getEuphoriaGalleryItems, saveEuphoriaGalleryItem, deleteEuphoriaGalleryItem,
} from '../../../../lib/euphoriaGalleryService';
...
const CATEGORIES: { id: EuphoriaGalleryCategory; label: string }[] = [
...
const BLANK: Omit<EuphoriaGalleryItem, 'id' | 'createdAt' | 'updatedAt'> = {
  title: '', type: 'photo', url: '', thumbnailUrl: '',
...
function GalleryItemForm({
  ...
  initial: Partial<EuphoriaGalleryItem>;
  onSave:  (data: Omit<EuphoriaGalleryItem, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }, file: File | null) => void;
  onCancel: () => void;
  saving: boolean;
}) {
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-5">
        ...
        <div className="flex gap-2">
          {(['photo', 'video'] as const).map(t => (
            <button
              ...
              style={form.type === t ? { background: PINK, color: '#fff' } : { background: '#F3F4F6', color: '#6B7280' }}
            >
              {t === 'photo' ? '📷 Photo' : '🎬 Video'}
            </button>
          ))}
      ...
      {form.type === 'photo' ? (
        <div>
          ...
          {preview ? (
            <div className="relative mb-2">
              ...
              <button
                type="button"
                onClick={() => { setPreview(''); set('url', ''); setPendingFile(null); }}
            ...
            <div
              className="border-2 border-dashed border-gray-300 rounded-xl p-6 text-center cursor-pointer hover:border-[#CE1868] transition-colors"
        ...
        <div>
          <label className={S.label}>YouTube URL or Video ID *</label>
          <input
            value={form.youtubeId}
            onChange={e => {
              const raw = e.target.value.trim();
              // Extract video ID from any YouTube URL format, or use as-is if already an ID
              const match = raw.match(
                /(?:youtu\.be\/|youtube\.com\/(?:watch\?(?:.*&)?v=|embed\/|shorts\/))([A-Za-z0-9_-]{11})/
              );
              set('youtubeId', match ? match[1] : raw);
            }}
            className={S.input}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <input
            ...
            className={S.input}
            placeholder="e.g. World Latin Dance Cup 2024"
          />
        </div>
        <div>
        ...
        <div className="flex items-center gap-3 pt-5">
          ...
          />
          <label htmlFor="el-gi-active" className="text-sm font-semibold text-gray-700">
...
export function EuphoriaGalleryAdminPage() {
  const [items,   setItems]   = useState<EuphoriaGalleryItem[]>([]);
  const [editing, setEditing] = useState<Partial<EuphoriaGalleryItem> | null>(null);
  ...
  const load = useCallback(() => {
    setLoading(true);
    getEuphoriaGalleryItems().then(setItems).catch(() => {}).finally(() => setLoading(false));
  }, []);
  ...
  async function handleSave(
    data: Omit<EuphoriaGalleryItem, 'id' | 'createdAt' | 'updatedAt'> & { id?: string },
    file: File | null,
  ) {
    setSaving(true);
    try {
      await saveEuphoriaGalleryItem(data, file);
      setEditing(null);
      load();
    } catch { /* noop */ } finally {
      setSaving(false);
  ...
  async function handleDelete(id: string) {
    ...
    await deleteEuphoriaGalleryItem(id);
    load();
  }

  async function toggleActive(item: EuphoriaGalleryItem) {
    await saveEuphoriaGalleryItem({ ...item, isActive: !item.isActive });
    load();
  }
    ...
    <div className={S.page}>
      ...
      <div className="bg-pink-50 border border-pink-200 rounded-xl px-5 py-4 mb-6">
        ...
        <p className="text-xs text-pink-700">
          This feature requires an <code className="bg-pink-100 px-1 rounded">euphoria_gallery_items</code> table in Supabase.
          Create it with the same schema as <code className="bg-pink-100 px-1 rounded">kids_gallery_items</code> (without the <code className="bg-pink-100 px-1 rounded">title_es</code> column).
        </p>
      </div>
              ...
              <div key={item.id} className={`${S.card} group`}>
                <div className="relative w-full h-36 bg-gray-100">
                  {thumb ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-3xl" style={{ backgroundColor: '#4A0030' }}>
                      {item.type === 'photo' ? '📷' : '🎬'}
                    </div>
                  )}
                  {item.type === 'video' && (
                ...
                <div className="p-3">
                  {item.title && (
                    <p className="text-xs font-bold text-gray-700 truncate mb-1">{item.title}</p>
                  )}
                  <p className="text-[10px] text-gray-400 uppercase mb-2">{item.category}</p>
                  <div className="flex items-center gap-1.5">
                    ...
                    <button
                      ...
                      className="flex-1 py-1 rounded text-xs font-bold bg-gray-100 hover:bg-gray-200 text-gray-700 transition-colors"
                    >
                      Edit
                    </button>
                    <button
